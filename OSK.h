...
#define KGRID (VMAX/KCOLS) // 480/14 = 22. "-3" = Leave some space for right menu buttons.
#define KLEFT 2 //((HMAX - KGRID * KCOLS)/2) // left offset to center keyboard
#define KROUND 3
#define KCOL 0xfff8
#define KRCOL 0x1111
#define FITS_ON_LINE 36
#define OSKBORDERH KLEFT
#define OSKBORDERV 20

void oskStart(char * buf, uint16_t ypos, int setVal);
void oskStop (void);
...
{	
  ...
  tft.setRotation(screenCalOSK[5]);
  tft.setTouch(screenCalOSK);
  oskY = ypos;
  oskBuf = buf;   // mainstream code needs to display the updated buffer
  oskBCnt = strlen(buf);
  oskShift = false;  
  oskOn = true;
  maxText = setVals[setVal].pre;
  Serial.printf("OSK: KGRID %i, y %i, max %i\n", KGRID, oskY, maxText);
  // display keyboard
  drawOSK();
  delay(500);  // avoid hang-over screen touch
  wasTouched = false;
}
void oskStop (void){
   oskOn = false;   
...
void drawOSKEditLine(void){
  ...
  tft.setTextFont(0); // revert to GLCD font for larger character range
...
void drawOSK(void){
 ...
 // tft.getTextBounds("A", 0,0, &xdum, &ydum, &cwid, &cht); 
  ...
  //tft.fillRect(KLEFT , oskY, KCOLS*KGRID, KGRID, MIDBG);
  tft.fillRect(0 , oskY - OSKBORDERV, KCOLS*KGRID + 2* OSKBORDERH, (KROWS+1)*KGRID + 2 *OSKBORDERV, OSKBG); 
  //tft.setCursor(KLEFT + XOFF , oskY + YOFF);
  drawOSKEditLine();
  //tft.print(oskBuf);
  for (j=0; j < KROWS ; j++)
  {
    ...
    for (i = 0; i < KCOLS; i++)
    {
      ...
      {   
		...
		#ifdef ILI9488
		   tft.setTextSize(2);
		   tft.drawChar(ch, xpos + XOFF, ypos + YOFF,  GFXFF);
		#else
...
int oskProcess(int cmd)
{
  ...
  for (j=0; j < KROWS ; j++)
  {
    ...
    { 
      ...
      if (     p.x > xpos && p.x < (xpos + KGRID) 
            && p.y > ypos && p.y < (ypos + KGRID)
          )
      {    
		    foundOne = true;
        chIndex = i + j * KCOLS;
        Serial.printf("OSK block [%i,%i] ", p.x, p.y); // p. provided by processTouchSwEnc()
        Serial.println(kb[chIndex].c); 
